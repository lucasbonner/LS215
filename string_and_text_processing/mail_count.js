...
const DATES = {0: 'Sun', 1:'Mon', 2: 'Tues', 3: 'Wed', 4:'Thu', 5:'Fri', 6:'Sat'};
const MONTHS = {0: 'Jan', 1: 'Feb', 2: 'Mar', 3: 'Apr', 4: 'May', 5: 'Jun', 6: 'Jul', 7: 'Aug', 8: 'Sep', 9: 'Oct', 10: 'Nov', 11: 'Dec'};

function dateConverter(elem) {
...
function mailCount(emailData) {
  let arrayOfMessages = emailData.split("##||##");
  let emails = {};
  let counter = 0;
  arrayOfMessages.forEach((elem) => {
    emails[counter] = elem.split('#/#')[2];
    counter++;
  });
  let dates = [];
  for (let prop in emails) {
    dates.push(emails[prop].split(':')[1]);
  }
  dates = dates.sort();
  dates = dates.map(dateConverter);
  console.log(`Count of Email: ${counter}`);

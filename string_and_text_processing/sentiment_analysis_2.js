...
function sentiment(text) {
  ...
  let positiveWordList = wordList.filter((elem) => {
    return elem.match(positiveRegex);
  });
  let negativeWordList = wordList.filter((elem) => {
    return elem.match(negativeRegex);
  });

  console.log(`There are ${positiveWordList.length} positive words in the text`);
  console.log(`Positive sentiments: ${positiveWordList.join(', ')}`);

  console.log(`There are ${negativeWordList.length} negative words in the text.`);
  console.log(`Negative sentiments: ${negativeWordList.join(', ')}`);

  let sentiment = "";
  if (negativeWordList.length > positiveWordList.length) {
    sentiment = "Negative";
  } else if (positiveWordList.length > negativeWordList) {
    sentiment = "Positive";
  }
  else {
    Sentiment = "Neutral";
